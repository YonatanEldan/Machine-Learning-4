...
def expectation(points_list, mu, sigma, w):
    ...
    """
    K = len(mu)
    # create the likelihood matrix and inititialize all the values to 0
    likelihood = np.zeros((len(points_list), K))
    ###########################################################################
    # TODO: Implement the function. compute likelihood array                  #
    ###########################################################################
    #i will be the index for a point in point list, j will be the index for mu,sigma and w.
    for i in range(len(points_list)):
        for j in range(K):
            likelihood[i][j] = (normal_pdf(points_list[i],mu[j],sigma[j])*w[j])        
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def maximization(points_list, ranks):
    ...
    ###########################################################################
    #W calculation
    w_new = np.sum(ranks, axis=0) / len(points_list)
    #mu calculation
    for j in range(K):
        for i in range(len(points_list)):
            mu_new[j] += ranks[i][j]*points_list[i]
        mu_new[j] = mu_new[j]/(w_new[j] * len(points_list))
    #sigma calculation
    for j in range(K):
        for i in range(len(points_list)):
            sigma_new[j] += ranks[i][j]*(np.square(points_list[i]-mu_new[j]))
        sigma_new[j] = sigma_new[j]/(w_new[j] * len(points_list))
...
def calc_max_delta(old_param, new_param):
    ...
    ###########################################################################
    max_delta = np.max(abs(old_param-new_param))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def expectation_maximization(points_list, k, max_iter, epsilon):
    ...
    while delta > epsilon and iter_num <= max_iter:
        ...
        iter_num += 1
        print(iter_num , ":" , w_new, mu_new, sigma_new )
